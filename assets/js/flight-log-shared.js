(function (global) {
  ...
  function injectStyle() {
    ...
    document.head.insertAdjacentHTML("beforeend", '<style id="flight-log-shared-style">' +
      '.fl-toolbar-btn{min-width:88px;height:32px;border-radius:6px;font-size:12px;font-weight:600}.fl-op-btn{color:#38bdf8;font-size:12px}.fl-op-btn:hover{color:#67e8f9;text-decoration:underline}.fl-search{height:32px}.fl-modal-mask{position:fixed;inset:0;background:rgba(2,8,23,.72);display:none;align-items:flex-start;justify-content:center;z-index:1200;padding:72px 20px 32px;overflow-y:auto;box-sizing:border-box}.fl-modal-mask.show{display:flex}.fl-modal{width:min(1260px,96vw);max-height:calc(100vh - 104px);margin:0 auto;overflow:auto;flex-shrink:0;background:#071b33;border:1px solid rgba(34,211,238,.22);border-radius:10px;box-shadow:0 18px 56px rgba(0,0,0,.45)}.fl-dot{width:10px;height:10px;border-radius:999px;display:inline-block}.fl-detail-grid{display:grid;grid-template-columns:330px 1fr;gap:20px}.fl-map-shell{position:relative}.fl-map{width:100%;height:520px;border-radius:8px;overflow:hidden;border:1px solid rgba(34,211,238,.14)}.fl-detail-link{color:#38bdf8;text-decoration:none}.fl-detail-link:hover{text-decoration:underline}.leaflet-container{background:#e8ecf1}@media(max-width:1180px){.fl-detail-grid{grid-template-columns:1fr}.fl-map{height:420px}}' +
    '</style>');
  }
  ...
  function statusHtml(status) {
    ...
    return '<div class="inline-flex items-center gap-2 ' + meta.text + '"><span class="fl-dot ' + meta.dot + '"></span>' + meta.label + "</div>";
  }
  ...
  function render(containerId, options) {
    ...
    root.innerHTML =
      ...
      '<div id="detail-modal" class="fl-modal-mask"><div class="fl-modal"><div class="flex items-center justify-between gap-3 px-6 py-4 border-b border-white/10"><h3 class="text-base font-semibold text-white">' + (opts.detailTitle || "飞行日志详情") + '</h3><button type="button" class="wh-modal-close" data-close="detail-modal" aria-label="关闭">×</button></div><div class="p-6"><div class="fl-detail-grid"><section><h4 class="text-sm font-semibold text-white mb-4">基本信息</h4><div id="detail-info" class="space-y-3 text-sm"></div></section><section><div class="flex flex-wrap items-center justify-between gap-3 mb-4"><h4 class="text-sm font-semibold text-white">飞行轨迹</h4><div class="flex flex-wrap gap-2"><button id="detail-play-track-btn" type="button" class="wh-btn-primary px-4 py-2 text-sm"><i class="fa-solid fa-play mr-1"></i>播放轨迹</button><button id="detail-map-reset-btn" type="button" class="wh-btn-ghost px-4 py-2 text-sm"><i class="fa-solid fa-location-crosshairs mr-1"></i>重置视角</button><button id="detail-export-btn" type="button" class="wh-btn-gold px-4 py-2 text-sm">导出</button></div></div><div class="fl-map-shell track-map-shell"><div id="detail-map" class="fl-map"></div><div class="track-map-panel"><div class="track-map-panel__row"><span class="track-map-panel__label">轨迹状态</span><span id="fl-map-status-text">待播放</span></div><div class="track-map-panel__row"><span class="track-map-panel__label">当前位置</span><span id="fl-map-current-name">--</span></div><div class="track-map-panel__row"><span class="track-map-panel__label">定位时间</span><span id="fl-map-current-time">--</span></div></div></div></section></div><div class="mt-6 pt-4 border-t border-white/10 flex justify-center"><button type="button" class="wh-btn-ghost px-6 py-2 text-sm" data-close="detail-modal">返回</button></div></div></div></div>';
    ...
    function openDetail(id) {
      ...
      setTimeout(function () {
        ...
        if (trackPlayback) trackPlayback.destroy();
    ...
    root.addEventListener("click", function (event) {
      ...
      if (event.target.closest("#detail-play-track-btn") && trackPlayback) trackPlayback.play();
      if (event.target.closest("#detail-map-reset-btn") && trackPlayback) trackPlayback.fitBounds();
    });
