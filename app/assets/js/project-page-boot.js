/**
 * 移动端项目管理：在静态脚本加载完成后启动 WHInProjectPage
 */
(function (global) {
  "use strict";
  ...
  function updateNavTitle(text) {
    ...
    if (el) el.textContent = text || "项目管理";
  }
  ...
  function patchViewNav() {
    ...
    function sync() {
      ...
      if (!detailEl.classList.contains("hidden")) {
        ...
        updateNavTitle(nameEl && nameEl.value ? nameEl.value : "项目详情");
        return;
      }
      updateNavTitle("项目管理");
    }
  ...
  function showBootError(msg) {
    if (msg) console.warn("[project-page]", msg);
  }
  ...
  function tryBoot() {
    ...
    if (!global.WHInProjectPage || typeof global.WHInProjectPage.boot !== "function") {
      showBootError("未找到 WHInProjectPage，请检查 assets/js/in-project-page.js 是否可访问。");
      return;
    }
    try {
